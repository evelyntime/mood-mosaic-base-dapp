...
const source = await readFile(resolve(process.cwd(), "contracts/MoodMosaic.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "MoodMosaic.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["MoodMosaic.sol"]?.MoodMosaic;
if (!contract) {
...
console.log(`Deploying MoodMosaic from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_MOOD_MOSAIC_CONTRACT_ADDRESS to this contract address in Vercel.");
